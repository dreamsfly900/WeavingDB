...
public unsafe class BPTree
{
    ...
    /// <returns></returns>
    public void insert(Node root, void* key, ListDmode value, byte _datatype)
    {
        datatype = _datatype;
    ...
    internal List<ListDmode> searcheQualto(Node root, void* v,byte databyte,int mtsContrast)
    {
        ...
        {
            int i = 0;
            while (!utli.CompareLogical(databyte, nd.keys[i].key, 2, v ))
            {
                i++;
            }
...
public unsafe class Node
{
    ...
    protected void insertOrUpdate(void* key,dynamic value)
    {
        //二叉查找，插入
        int low = 0, high = keys.Count - 1, mid;
        KV comp;
